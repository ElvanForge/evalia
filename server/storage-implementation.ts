...
export class DatabaseStorage implements IStorage {
  ...
  async getDashboardStats(teacherId: number): Promise<{
    ...
    const enrolledStudentsQuery = sql`
      ...
      FROM ${schema.studentClasses}
      WHERE ${schema.studentClasses.classId} IN (${sql.join(classIds, sql`, `)})
    `;
    ...
    const openAssignmentsQuery = sql`
      ...
      WHERE ${schema.assignments.classId} IN (${sql.join(classIds, sql`, `)})
      AND ${schema.assignments.dueDate} > ${now}
    `;
    ...
    const avgGradeQuery = sql`
      SELECT AVG(CAST(${schema.grades.score} AS DECIMAL) / CAST(${schema.assignments.maxScore} AS DECIMAL) * 100) as avg_grade
      FROM ${schema.grades}
      JOIN ${schema.assignments} ON ${schema.grades.assignmentId} = ${schema.assignments.id}
      WHERE ${schema.assignments.classId} IN (${sql.join(classIds, sql`, `)})
    `;
