import { pgTable, text, serial, integer, boolean, decimal, timestamp, primaryKey, jsonb } from "drizzle-orm/pg-core";
...
export const lessonPlans = pgTable("lesson_plans", {
  ...
  subject: text("subject"),
  gradeLevel: text("gradeLevel"),
  duration: text("duration"), // e.g., "45 minutes", "1 hour"
  objectives: text("objectives").array(),
  materials: text("materials").array(),
  content: text("content").notNull(), // Rich text content of the lesson plan
...
// Lesson Plan Materials (uploaded files)
export const lessonPlanMaterials = pgTable("lesson_plan_materials", {
  id: serial("id").primaryKey(),
  lessonPlanId: integer("lessonPlanId").notNull().references(() => lessonPlans.id),
...
export const insertLessonPlanMaterialSchema = createInsertSchema(lessonPlanMaterials).omit({
  id: true,
  uploadedAt: true,
});

// AI-generated content for lesson plans
export const lessonPlanGeneratedContent = pgTable("lesson_plan_generated_content", {
  id: serial("id").primaryKey(),
  lessonPlanId: integer("lessonPlanId").notNull().references(() => lessonPlans.id),
  prompt: text("prompt").notNull(), // The prompt used to generate the content
  generatedContent: text("generatedContent").notNull(), // The content generated by the AI
  contentType: text("contentType").notNull(), // objectives, activities, assessments, etc.
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  isApplied: boolean("isApplied").default(false), // Whether the content was applied to the lesson plan
});

export const insertLessonPlanGeneratedContentSchema = createInsertSchema(lessonPlanGeneratedContent).omit({
  id: true,
  createdAt: true,
});
...
export type InsertLessonPlanMaterial = z.infer<typeof insertLessonPlanMaterialSchema>;

export type LessonPlanGeneratedContent = typeof lessonPlanGeneratedContent.$inferSelect;
export type InsertLessonPlanGeneratedContent = z.infer<typeof insertLessonPlanGeneratedContentSchema>;
